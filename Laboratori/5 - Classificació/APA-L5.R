...
library(klaR)
(rda.model <- rda (Wine.type ~ ., data = wine,  lambda = 1))
...
# form and display confusion matrix & overall error
tab <- table(Pred=pred, True=HouseVotes84[-learn,]$Class) 
tab
1 - sum(tab[row(tab)==col(tab)])/sum(tab)
...
## So we get a much better spam filter; notice that the filter has a lower probability of 
